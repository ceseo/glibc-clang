...
static void
R_HEAP (void *pbase, void *end, size_t size, swap_t swap,
...
void
R_FUNC (void *const pbase, size_t total_elems, size_t size,
	R_CMP_TYPE cmp R_CMP_ARG)
{
  ...
  if (total_elems > MAX_THRESH)
    {
	  ...
	  if (R_CMP ((void *) hi, (void *) mid) < 0)
	    swap (mid, hi, size);
	  else if (R_CMP ((void *) mid, (void *) lo) < 0)
	    swap (mid, lo, size);
	  ...
	  if (depth-- == 0)
	    {
	      R_HEAP (left_ptr, right_ptr, size, swap, cmp R_CMP_ARG_USE);
	    }
